...
5. 导入分析结果并提交到独立开发分支，通过 GitHub PR 远程审核。
...
git switch -c codex/<weekly-task> origin/develop
```
...
10. 运行 Python 测试和站点构建，将报告提交到独立开发分支并只推送该分支。
11. 创建以 develop 为 base 的 Draft PR；最终必须停留在 review 状态，不得直接
    push develop/main 或触发正式 GitHub Pages。
12. 最后报告：部门、ISO 周次、采集状态、候选数、入选数、总阅读时间、来源失败、
    质量门禁、开发分支和远程审核 PR 地址。
...
把输出文件提交到独立开发分支，只 push 该分支并向 `develop` 创建 Draft PR。
`Build remote weekly review` 会上传只读站点 Artifact；研究员在 PR 中逐条检查题名、
摘要、问题、方法、结果、证据、限制和原始来源链接。
